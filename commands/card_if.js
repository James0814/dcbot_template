...
module.exports = {
    ...
    async execute(client, interaction) {
        ...
        let x = a * b - c * d; // 公式好像怪怪的，應該要如何更改呢?
        if (x < 0) {
            // 如何判斷 大或小
            await interaction.reply(`利奧拉的牌比較小 小${-x}`); // 如果符合，應該回覆甚麼訊息
        } else if (x = 0) {
            // 應該要如何判斷剩下的
            await interaction.reply(`兩人的牌一樣大 都是${x}`); // 如果符合，應該回覆甚麼訊息
        } else {
            // 最後一個應該是甚麼情況
            await interaction.reply(`利奧拉的牌比較大 大${x}`); // 如果符合，應該回覆甚麼訊息
        }
    },
};
